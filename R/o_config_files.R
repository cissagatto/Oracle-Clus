...
###############################################################################
FolderCF = paste(FolderRoot, "/O-Config-Files", sep="")
...
while(i<=n){
  ...
  # Confi File Name
  file_name = paste(FolderCF, "/O-", ds$Name, ".csv", sep="")
  ...
  write("Dataset_Path, ~/Oracle-Clus/Datasets",
        file = output.file, append = TRUE)

  # job name
  job_name = paste("o-", ds$Name, sep = "")
  ...
  write("Partitions_Path, ~/Oracle-Clus/BellPartitions",
        file = output.file, append = TRUE)
